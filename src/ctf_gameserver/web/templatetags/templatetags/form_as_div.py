...
def as_div(form):
    ...
    output = []
    top_errors = _ListGroupErrorList(
        [conditional_escape(e) for e in form.non_field_errors()]
    )
    ...
    for field in form.hidden_fields():
        ...
        for error in field.errors:
            top_errors.append(
                format_html(
                    _("Hidden field {name}: {error}"), name=field.name, error=error
                )
            )
        output.append(str(field))

    for field in form.visible_fields():
        div_classes, field_classes, iterate_subfields, wrap_in_label = _get_css_classes(
            field
        )

        div_class_string = format_html_join(
            " ", "{}", [(mark_safe(c),) for c in div_classes]
        )
        div_class_string += mark_safe(" ") + conditional_escape(
            force_str(field.css_classes())
        )
        field_class_string = format_html_join(
            " ", "{}", [(mark_safe(c),) for c in field_classes]
        )
        ...
        if field.errors:
            output.append(
                str(_ListGroupErrorList([conditional_escape(e) for e in field.errors]))
            )
        ...
        if iterate_subfields:
            ...
            for subfield in field:
                rendered_parts.append(
                    format_html('<div class="{}">', field_class_string)
                )
            ...
            rendered_field = force_str(
                field.as_widget(attrs={"class": field_class_string})
            )
        ...
        if wrap_in_label:
            label_content = (
                rendered_field
                + mark_safe(" ")
                + conditional_escape(force_str(field.label))
            )
        ...
        if not field.field.required:
            help_text = (
                mark_safe('<span class="label label-default">Optional</span> ')
                + help_text
            )
