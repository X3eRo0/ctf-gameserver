...
from ctf_gameserver.web.scoring.decorators import (
...
class TeamList(ListView):
    ...
    queryset = Team.active_not_nop_objects.select_related("user").order_by(
        "user__username"
    )
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if user_form.is_valid() and team_form.is_valid():
            ...
            messages.success(
                ...
                mark_safe(
                    _(
                        "Successful registration! A confirmation mail has been sent to "
    ...
    return render(
        request, "register.html", {"user_form": user_form, "team_form": team_form}
    )
...
def edit_team(request):
    ...
    if request.method == "POST":
        ...
        team_form = forms.TeamForm(
            request.POST, request.FILES, prefix="team", instance=team
        )
        ...
        if user_form.is_valid() and team_form.is_valid():
            ...
            if "email" in user_form.changed_data:
                ...
                messages.warning(
                    request,
                    _(
                        "A confirmation mail has been sent to your new formal email "
...
def delete_team(request):
    ...
    return render(
        request,
        "edit_team.html",
        {"user_form": user_form, "team_form": team_form, "delete_form": delete_form},
    )
...
def confirm_email(request):
    ...
    try:
        ...
        messages.error(
            request, _("Missing parameters, email address could not be confirmed.")
        )
    ...
    if game_control.min_net_number is None or game_control.max_net_number is None:
        ...
        possible_net_numbers = set(
            range(game_control.min_net_number, game_control.max_net_number + 1)
        )
    ...
    while True:
        try:
            ...
            with transaction.atomic():
                ...
                try:
                    random_net_number = random.choice(
                        list(possible_net_numbers)
                    )  # nosec
                except IndexError:
                    logging.error(
                        "Net numbers exhausted, could not confirm team (ID) %d", user_pk
                    )
    ...
    user.save()
    messages.success(
        request, _("Email address confirmed. Your registration is now complete.")
    )
...
def mail_teams(request):
    ...
    if form.cleaned_data["addrs"] == "formal":
        addresses = [
            values["user__email"]
            for values in Team.active_objects.values("user__email").distinct()
        ]
    else:
        addresses = [
            values["informal_email"]
            for values in Team.active_objects.values("informal_email").distinct()
        ]
