...
def main():
    ...
    arg_parser.add_argument(
        "--metrics-listen", help='Expose Prometheus metrics via HTTP ("<host>:<port>")'
    )
    ...
    try:
        ...
        database.add_capture(
            db_conn, 2147483647, 1, prohibit_changes=True, fake_team_id=42, fake_tick=1
        )
...
def make_metrics(registry=prometheus_client.REGISTRY):
    ...
    counters = [
        ...
        (
            "flags_err",
            "Number of submitted flags which resulted in an error",
            ["team_net_no"],
        ),
        (
            "server_kills",
        ...
        (
            ...
            "Number of unexpected exceptions in client connections",
            [],
        ),
    ]
    for name, doc, labels in counters:
        metrics[name] = prometheus_client.Counter(
            metric_prefix + name, doc, labels, registry=registry
        )
    ...
    for name, doc, labels in histograms:
        ...
        metrics[name] = prometheus_client.Histogram(
            metric_prefix + name, doc, labels, registry=registry
        )
...
async def serve(host, port, db_conn, params):
    ...
    async def wrapper(reader, writer):
        ...
        try:
            ...
            logging.warning(
                "[%s]: Client connection error, closing the connection", client_addr
            )
            writer.close()
        except:  # noqa, pylint: disable=bare-except
            logging.exception(
                "[%s]: Exception in client connection, closing the connection:",
                client_addr,
            )
...
async def handle_connection(reader, writer, db_conn, params):
    ...
    try:
        ...
        logging.error(
            "[%s]: Could not match client address with team, closing the connection",
            client_addr,
        )
    ...
    try:
        await handle_team_connection(
            reader, writer, db_conn, params, client_addr, client_net_no
        )
    finally:
        metrics["open_connections"].labels(client_net_no).dec()


async def handle_team_connection(
    reader, writer, db_conn, params, client_addr, client_net_no
):
    """
    ...
    log(
        "INFO",
        "Accepted connection from %s (team net number %d)",
        client_addr,
        client_net_no,
    )

    writer.write(
        f'{params["competition_name"]} Flag Submission Server\n'.encode("utf-8")
    )
    ...
    while True:
        ...
        try:
            flag_id, protecting_net_no = flag_lib.verify(
                flag, params["flag_secret"], params["flag_prefix"]
            )
            ...
            log(
                "INFO",
                "Flag %s rejected because it has expired since %s",
                repr(flag),
        ...
        if protecting_net_no == client_net_no:
            ...
            log(
                "INFO",
                "Flag %s rejected because it is protected by submitting team",
                repr(flag),
            )
        ...
        try:
            ...
            if now < start:
                ...
                log(
                    "INFO",
                    "Flag %s rejected because competition has not started",
                    repr(flag),
                )
            ...
            if database.team_is_nop(db_conn, protecting_net_no):
                ...
                log(
                    "INFO",
                    "Flag %s rejected because it is protected by a NOP team",
                    repr(flag),
                )
                metrics["flags_inv"].labels(client_net_no).inc()
                continue
            ...
            try:
                ...
                log(
                    "INFO",
                    "Flag %s rejected because it has already been submitted before",
                    repr(flag),
                )
                ...
                log(
                    "WARNING",
                    "Flag %s: Could not find team for net number %d in database",
                    repr(flag),
                    client_net_no,
                )
                metrics["flags_err"].labels(client_net_no).inc()
